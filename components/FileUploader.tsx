...
export const FileUploader: React.FC<Props> = ({ onSlidesLoaded }) => {
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={`flex-1 border-2 border-dashed rounded-xl flex flex-col items-center justify-center p-8 transition-colors text-center cursor-pointer ${
    ...
    >
      ...
      {loading ? (
        ...
        <>
          ...
          <p className="font-bold text-slate-300">PDFをドラッグ＆ドロップ</p>
          <p className="text-xs text-slate-500 mt-2">またはクリックして選択 (最大10枚まで解析)</p>
        </>
      )}
    </div>
  );
